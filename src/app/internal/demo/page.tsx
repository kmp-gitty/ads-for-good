...
// Chapter Moment Identity — Sales Demo Playground (moved from /demo).
//
...
// using the seeded adsforgood_prod tenant. Ideal for sales-demo screen-shares —
// prospects see the modals fire against a realistic set of buttons + a live
// event feed.
//
// NOTE: This uses adsforgood_prod, which is the SAME tenant the public
// ads4good.com marketing site pixel loads with. Prompts under adsforgood_prod
// that match selectors on public pages WILL fire on public pages. Use the
// isolated /internal/prompt-playground (chapter_practice tenant) when you're
// iterating on new prompt configurations — that's the safer authoring surface.
//
// Pixel injection: ChapterLoader skips /internal/*, so we inject the
// adsforgood_prod pixel manually here. This preserves the original /demo
// behavior after the move (the page still POSTs events under adsforgood_prod).
...
const PRESETS: Preset[] = [
  ...
  },
  {
    ...
    extraNotes:
      "Live thresholds seeded: global 85% + product mock-hoodie-001 at 60% / $60 floor. Bid $60+ → auto-accept + real CHAPTER-XXXXXXXX code via shopify-mock. Bid $53-59 → counter-offer. Bid <$53 → decline.",
  },
  {
    id: "phone_call",
    name: "Phone Call",
...
export default function InternalDemoPage() {
  ...
  // Manually inject the pixel with adsforgood_prod (ChapterLoader is skipped
  // on /internal/*). Idempotent via the pixel's own __chapterLoaded guard.
  useEffect(() => {
    if (document.querySelector('script[data-demo-pixel="true"]')) return;
    const script = document.createElement("script");
    script.src = "/api/chapter/pixel.js";
    script.async = true;
    script.setAttribute("data-demo-pixel", "true");
  ...
  return (
    <div className="space-y-6">
      {/* Header nav strip within the internal layout */}
      ...
      {/* Footer note */}
      <footer className="mt-6 rounded-lg border border-dashed border-neutral-300 bg-white p-4 text-xs text-neutral-600">
        <p className="font-semibold text-neutral-800">Notes</p>
        <ul className="mt-1 list-disc space-y-1 pl-5">
          <li>
            Pixel loads on this page with{" "}

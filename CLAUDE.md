...
> Last updated: July 9, 2026 (MI v2 Phase 5 — Make an Offer SHIPPED end-to-end. Followup to July 5 planning where Path A (Shopify-focused, adapter-ready) was locked. This session executes it: platform adapter layer (shopify + shopify-mock + square + custom stubs + selector) with `PLATFORM_ADAPTER_MODE` env-driven mock/real routing; offers logic module (types + state-machine DAG + evaluator with product/collection/global/fallback threshold hierarchy); `POST /api/chapter/offer-submit` with the 3-layer defense pattern (honeypot + HMAC session token + 10/hr per-IP rate limit) that auto-accepts / counters / declines / routes-to-review; admin review queue at `/internal/identity-prompts/<key>/offers` with inline approve/counter/decline actions gated by chapter_staff role + state-machine transitions; threshold config UI at `/internal/identity-prompts/<key>/offer-thresholds`; pixel `make_an_offer` preset renderer with bid + email + decision-appropriate success state. Mocked-adapter dev pattern works end-to-end without a live Shopify store — swap in real credentials post-signing to activate the real adapter path with zero code changes.)

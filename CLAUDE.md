...
- **What Phase 5.7 explicitly deferred:**
  - Preset-specific admin form builder — v1 assumes operator ships the modal with hardcoded target via `container_jsonb.target` on the prompt row. A dedicated MakeAnOfferBuilder mirroring CustomFormBuilder is the natural next step but not blocking for the Shopify sales conversation

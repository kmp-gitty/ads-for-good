...
> Last updated: July 2, 2026 (evening — NSC Square webhook path recovered from silent failure since June 10 onboarding. Root cause: Vercel apex `ads4good.com` 307-redirects to `www.ads4good.com`; Square doesn't follow redirects on POST. Compound issue: our stored `notification_url` in `chapter_config.square_webhook_secrets` didn't match Square's actual signing URL after the fix, causing HMAC mismatch. Fixed both. Backfilled 923 bookings + 1 refund via Square API; ~4297 payments still running in bg. Full year of NSC attribution recovered.)

...
- **Pieces shipped:**
  ...
  - **`chapter_config.square_oauth_tokens`** (new table) — per-merchant Square OAuth access tokens with environment column ('sandbox' / 'production') so the helper picks the right API base URL. Includes `refresh_token` + `expires_at` slots for future automation; tokens expire ~30 days. Manual rotation for now; refresh automation added to backlog.

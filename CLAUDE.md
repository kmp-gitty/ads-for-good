...
- **Functions NOT modified.** `lifecycle_chapters_incremental` and `refresh_canonical_v2_snapshot` keep their hardcoded `v_date_floor`. This is intentional — Fix 1A ships the foundation, Fix 1B (future) will swap the hardcoded floor for `chapter_config.retention_floor()` with the chapter-spanning caveat (per-canonical effective_floor + chapter_id continuity). Doing 1B tonight would stack a third chain change on top of Fix 2 + chain-order; waiting until tonight's run validates is the safer sequence.
